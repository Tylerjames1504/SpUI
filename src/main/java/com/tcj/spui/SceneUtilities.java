...
public class SceneUtilities {
  ...
  public void swapScene(String sceneKey, String nextTitle) {
    if (nextTitle != null) {
      parentStage.setTitle(nextTitle);
    }
    parentStage.setScene(App.session.getStageManager()
        .retrieveValidChildSceneFromStageParent(parentStageKey, sceneKey));
  }
  ...
  public void swapToStage(Boolean closeOldStage, String stageKey, String sceneKey) {
    ...
    Stage nextStage = App.session.getStageManager().retrieveStageSubNetworkWithKey(stageKey)
        .getParentStage();
    nextStage.setScene(
        App.session.getStageManager().retrieveValidChildSceneFromStageParent(stageKey, sceneKey));
    nextStage.show();
  }
  ...
  } // bug with button staying bold because of bad hover functionality
